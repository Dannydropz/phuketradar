...
- **Duplicate Detection**: Six-layer system with database-level protection:
    ...
    3. **Facebook Post ID check (NEW - Oct 29, 2025)**: Queries database by canonical facebook_post_id before translation to prevent duplicates from Facebook's multiple ID formats. The extractFacebookPostId() function normalizes both pfbid and numeric IDs to a single canonical value (numeric preferred), ensuring posts like `/posts/1146339707616870` and `/posts/pfbid0wGs...` are recognized as the same article. Saves API costs by blocking duplicates before expensive translation.
    4. **Database source URL check**: Queries database before translation to avoid expensive API calls for known posts
    5. **Multi-image duplicate detection**: Checks ALL images in imageUrls array against existing articles. If any image matches an existing article, the post is skipped. This catches same stories from different Facebook sources that share images (even if they use different primary images or post order).
    6. **Semantic similarity check**: 70% threshold on Thai title embeddings to catch near-duplicate stories with different wording
    7. **Database UNIQUE constraints**: PostgreSQL UNIQUE constraints on both `articles.source_url` and `articles.facebook_post_id` prevent race condition duplicates (error code 23505 handled gracefully)
    
    **Production Setup**: After publishing, add UNIQUE constraints to production database:
    1. Open Replit Database UI → Navigate to production database
    2. Go to Database tab → Select `articles` table
    3. Check for source_url duplicates: `SELECT source_url, COUNT(*) FROM articles GROUP BY source_url HAVING COUNT(*) > 1;`
    4. Check for facebook_post_id duplicates: `SELECT facebook_post_id, COUNT(*) FROM articles WHERE facebook_post_id IS NOT NULL GROUP BY facebook_post_id HAVING COUNT(*) > 1;`
    5. Clean duplicates if found (delete duplicate rows keeping only the earliest one)
    6. Run SQL to add both constraints:
       ```sql
       ALTER TABLE articles ADD CONSTRAINT articles_source_url_unique UNIQUE (source_url);
       ALTER TABLE articles ADD CONSTRAINT articles_facebook_post_id_unique UNIQUE (facebook_post_id);
       ```
    7. If backfill is needed: Run `npx tsx scripts/backfill-facebook-post-ids.ts` to populate facebook_post_id from existing source URLs
    8. If correction is needed: Run `npx tsx scripts/fix-facebook-post-ids.ts` to normalize existing IDs to canonical format
